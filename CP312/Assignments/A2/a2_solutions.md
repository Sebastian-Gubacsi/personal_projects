# CP 312 - Assignment 2 Solutions
## Fall 2025

---

## Question 1: 2-Way-2-Sum Problem [6 marks]

### Main Idea

The brute force approach of checking all quadruples of indices requires Θ(n⁴) time. A reduction to 3-Sum yields Θ(n³). To achieve o(n³), we transform the problem into a set intersection problem: compute all possible pairwise sums from each array (Θ(n²) sums each) and check if any sum appears in both sets. Using a hash-free approach with sorting and two-pointer technique, we can determine if the arrays share a common sum in Θ(n² log n) time.
...
Algorithm 2-Way-2-Sum(A, B, n):
    // Compute all pairwise sums from array A
    ...
    // Compute all pairwise sums from array B
...
## Question 2: Finding Maximum in Circularly Shifted Sorted Array [6 marks]

### Main Idea

After k applications of the CRS operation to a sorted array, there exists exactly one position where the sorted order "breaks" — where a larger element is immediately followed by a smaller element. The maximum element is located at this break point. We can locate this position using binary search by examining the relationship between elements at the midpoint and the boundaries, determining which half contains the break point. This yields O(log n) time complexity.
...
**Termination:** The loop terminates when we find the maximum (Cases 1, 2) or when the range contains ≤ 2 elements, at which point we return the larger.
...
## Question 3: Recurrence Relations [6 marks]
...
We construct a recursion tree where each node represents a subproblem:
...
## Question 4: Station Load Factor Problem [8 marks]

### Main Idea

The key insight is that after sorting stations by x-coordinate, the constraint xᵢ < xⱼ is automatically satisfied for all pairs where i < j in the sorted order. The problem then reduces to counting, for each station i, how many stations j > i have yⱼ < yᵢ. This is equivalent to counting inversions in the y-coordinate sequence. We use a divide-and-conquer approach based on merge sort that counts inversions while sorting, achieving O(n log n) time complexity.
...
## Question 5: Recursive Algorithm Analysis [4 marks]
...
| Question | Algorithm | Time Complexity | Requirement Met |
|----------|-----------|-----------------|-----------------|
| 1 | 2-Way-2-Sum | Θ(n² log n) | ✓ (o(n³)) |
| 2 | Circular Shift Max | O(log n) | ✓ (o(n)) |
| 3a | Recurrence (Tree) | Θ(n² log n) | — |
| 3b | Recurrence (Master) | Θ(n² log n) | — |
| 4 | Station Load Factor | O(n log n) | ✓ (o(n²)) |
| 5a | Fiction Worst Case | Θ(n log n) | — |
| 5b | Fiction Best Case | Θ(n) | — |

---

*End of Assignment 2 Solutions*