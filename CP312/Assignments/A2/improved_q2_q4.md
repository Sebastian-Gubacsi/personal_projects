...
2.  \[6 marks\] **Divide-and-Conquer/Binary Search.** Given a sorted in
    ...
    **Maintenance:** We show the invariant is preserved in each case:
    ...
    - **Case 3:** If X[mid] > X[right], the portion [mid, right] is not entirely sorted in ascending order (since if it were, we would have X[mid] ≤ X[right]), so the break point must be in (mid, right]. We set left ← mid + 1.
    - **Case 4:** If X[mid] ≤ X[right], the portion [mid, right] is sorted in ascending order (since X[mid] ≤ X[right] and the array is originally sorted with one rotation), so the break point must be in [left, mid-1]. We set right ← mid - 1.
    ...
    **Example:** Consider array [5, 6, 7, 1, 2, 3, 4] (originally [1,2,3,4,5,6,7] with 3 CRS operations).
    - Iteration 1: left=1, right=7, mid=4, X[4]=1, X[5]=2, X[3]=7. Check X[mid]=1 < X[mid-1]=7, so mid-1 is the break point. Return X[3]=7. ✓
...
4.  \[8 marks\] **Divide-and-conquer** The input for this problem
    ...
    **Pseudocode:**
    ...
    Algorithm MaxLoadFactor(stations, n):
        // Input: stations is an array where stations[i] = (xᵢ, yᵢ, originalID=i)
        // This ensures we can track which station each load factor belongs to
    ...
    Algorithm MergeAndCount(Y, indices, loadFactors, left, mid, right):
        ...
        while i ≤ mid and j ≤ right do
            if Y[i] > Y[j] then
                // Y[i] forms inversions with Y[j], Y[j+1], ..., Y[right]
                // (since right subarray is sorted, all are < Y[i])
                // This counts transmissions from station indices[i]
    ...
    *Proof of Lemma 3:* We prove by strong induction on the size of the array.
    - **Base case:** For arrays of size ≤ 1, there are no inversions. The algorithm correctly returns without counting.
    - **Inductive hypothesis:** Assume the algorithm works correctly for all arrays of size < n.
    - **Inductive step:** For an array of size n, inversions are partitioned into three disjoint sets:
        1. Inversions within the left half [left, mid] (counted by recursive call)
        2. Inversions within the right half [mid+1, right] (counted by recursive call)
        3. Cross-inversions where i ∈ [left, mid] and j ∈ [mid+1, right]
        
        By the inductive hypothesis, sets (1) and (2) are counted correctly.
        
        For set (3), during the merge phase: When Y[i] > Y[j] where i is from the left half and j is from the right half, since the right subarray [mid+1, right] is already sorted (by the inductive hypothesis), we have Y[j] ≤ Y[j+1] ≤ ... ≤ Y[right]. Therefore Y[i] > Y[j], Y[i] > Y[j+1], ..., Y[i] > Y[right], giving exactly (right - j + 1) inversions for position i. The algorithm correctly adds this count to loadFactors[indices[i]].
        
        Each cross-inversion is counted exactly once (when the larger element from the left is selected), so all three types are counted correctly and exactly once. ∎
    ...
    **Concrete Example:**
    Consider 5 stations: {(1,5,A), (2,3,B), (3,7,C), (4,2,D), (5,1,E)}

    After sorting by x-coordinate: [(1,5,A), (2,3,B), (3,7,C), (4,2,D), (5,1,E)]
    Extract Y-array: [5, 3, 7, 2, 1]
    Station indices: [A, B, C, D, E]

    Counting inversions (transmissions):
    - Position 0 (station A, y=5): Y[0]=5 > Y[1]=3, Y[3]=2, Y[4]=1 → 3 transmissions
    - Position 1 (station B, y=3): Y[1]=3 > Y[3]=2, Y[4]=1 → 2 transmissions  
    - Position 2 (station C, y=7): Y[2]=7 > Y[1]=3, Y[3]=2, Y[4]=1 → 3 transmissions
    - Position 3 (station D, y=2): Y[3]=2 > Y[4]=1 → 1 transmission
    - Position 4 (station E, y=1): No transmissions → 0 transmissions

    Maximum load factor: 3 (achieved by stations A and C) ✓
